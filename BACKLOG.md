...
| ~~P1~~ | ~~**PDP hero trust by product kind**~~ | **DONE Local (2026-06-03):** child `35a6fc9`; course/diagnostic/service trust copy, especially `video_plan` and `testmyself`. Woo `_atmo_*` meta consistency remains a separate operator/CMS pass. |
| P1 | **PDP CMS description dedupe (operator)** | **Sheet ready (2026-06-03):** `PDP_CMS_CLEANUP_CHECKLIST.md` — remove legacy who/FAQ/aggressive claims from Woo «Описание» after theme registry. **Seam polish DONE Local:** child `05fc665` CSS + `testmyself` diagnostic short-info (no WP Admin in that slice). |
