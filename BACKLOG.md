...
| ~~P1~~ | ~~**PDP FAQ/who implementation**~~ | **DONE Local (2026-06-03):** pilot `b7e5556` + wave 2 `dde8dcf`, registry **18/18** in `inc/atmo-pdp-content.php`. Gift FAQ still blocked by policy. Stage/prod deploy pending. |
| ~~P1~~ | ~~**PDP hero trust by product kind**~~ | **DONE Local (2026-06-03):** child `35a6fc9`; course/diagnostic/service trust copy, especially `video_plan` and `testmyself`. |
| ~~P1~~ | ~~**PDP CMS description dedupe (operator)**~~ | **DONE Local + Stage2 (2026-06-04):** wave 1 (2026-06-03) + wave 2a CMS (5 slugs) + `_atmo_*` meta. Checklist: `PDP_CMS_CLEANUP_CHECKLIST.md`. Stage2 transfer Wave 2a+2b PASS; see `CHANGES.md`. **Open outside this item:** owner-decision slugs (`facial_support`, `testmyself`, `video_plan`, pregnancy meta on `kurs-vse-o-beremennosti`) and P2 tone on slugs not in wave 2a. |
| ~~P1~~ | ~~**Stage2 transfer — Local PDP CMS + `_atmo_*` meta**~~ | ✅ **Closed 2026-06-04:** Stage2 child **0.1.3**, CMS stale cleanup PASS, full PDP meta parity **40 rows**, hero pills **10/10**, `/каталог/` card snippets PASS. **Not** a production deploy; production still blocked by post-snapshot safety gate (`DEPLOY_CHECKLIST.md` §0). Artifacts under `C:\tmp\atmo-handoff\`. |
...
| Item | Notes |
|------|-------|
| ~~**Section 04 Social** — testimonials~~ | ✅ `db25c75` + `b6e4566` — real approved Woo reviews, no hardcoded testimonials, newest-first with reviewer/product uniqueness. |
...
## 6a. Static / legal — reference

| Item | Notes |
|------|-------|
| `/trainer/` | **Removed from scope** 2026-06-01; `trainer.html` deleted from prototype repo. Do not implement unless product scope changes. |
