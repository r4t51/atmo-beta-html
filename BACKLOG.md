...
| ~~P1~~ | ~~**PDP CMS description dedupe (operator)**~~ | **Mostly DONE Local (2026-06-04):** wave 1 (2026-06-03) + wave 2a CMS (5 slugs) + `_atmo_*` meta (10 products / 12 fields). Checklist: `PDP_CMS_CLEANUP_CHECKLIST.md`. **Open:** owner-decision slugs (`facial_support`, `testmyself`, `video_plan`, pregnancy meta on `kurs-vse-o-beremennosti`); P2 tone on slugs not in wave 2a; **stage2 transfer pending** (not production deploy). |
| P1 | **Stage2 transfer — Local PDP CMS + `_atmo_*` meta** | Re-apply or migrate wave 2a tab edits + 12 meta fields on stage2; re-QA hero pills + `/каталог/` card snippets. **Not** production until post-snapshot safety gate (`DEPLOY_CHECKLIST.md` §0). Artifacts under `C:\tmp\atmo-handoff\`. |
