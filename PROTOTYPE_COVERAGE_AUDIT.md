...
| `product.html` | `/product/{slug}/` via `woocommerce/content-single-product.php` + `atmo-product.css` | `partial` | **Micro-parity PASS_LOCAL (2026-06-02):** pass 3 CSS. **PDP who/FAQ PASS_LOCAL (2026-06-03):** theme registry `inc/atmo-pdp-content.php` — **18/18** approved slugs (`b7e5556` pilot + `dde8dcf` wave 2). | Gift CTA/FAQ still blocked; prototype «Не подходит» column not implemented; global trust «кабинет» copy on service SKUs (e.g. `video_plan`) — separate polish; variable bottom CTA hidden via CSS (Snippet 12). |
