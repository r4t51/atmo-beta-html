...
| `course-complete.html` | Course completion celebration (account context) | `blocked-by-fixture` | Adapter detects `status=completed` (`learndash_course_completed` / LD progress) but hub has no celebration UI; no Local user with completed course for QA (fixture **r4t5** / course **3616** = active, 0% progress). | **Planned Option B:** completed-state panel in `/my-account/my-courses/?course_id={id}` (`atmo_render_course_hub` when `completed`). Review CTA → canonical Woo `{product_permalink}#reviews` (grant `product_id` / reverse `_related_course`); active access is enough for CTA, including manual LD enrollment. Stats from real outline/progress only. **Not Option C** (prototype `active: account`, not standalone route). Requires disposable completion fixture before skin QA. |
...
- Required prototype screens remain `blocked-by-fixture`: course completion (`course-complete.html` — needs Local completed-course user before hub skin).
