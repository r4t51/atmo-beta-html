...
| `product.html` | `/product/{slug}/` via `woocommerce/content-single-product.php` + `atmo-product.css` | `partial` | **Micro-parity PASS_LOCAL (2026-06-02):** pass 3 CSS. **PDP who/FAQ PASS_LOCAL (2026-06-03):** theme registry `inc/atmo-pdp-content.php` — **18/18** approved slugs (`b7e5556` pilot + `dde8dcf` wave 2). **Hero trust PASS_LOCAL:** `35a6fc9` kind-aware copy for course / diagnostic / service. | Gift CTA/FAQ still blocked; prototype «Не подходит» column not implemented; Woo `_atmo_*` meta consistency needs operator/CMS pass; variable bottom CTA hidden via CSS (Snippet 12). |
